...
int main (int argc, char **argv)
{
	...
	const uint64_t dim0 = atoll(argv[2]);
	const uint64_t dim1 = atoll(argv[3]);
	const uint64_t chunkdim0 = atoll(argv[4]);
	const uint64_t chunkdim1 = atoll(argv[5]);
	...
	//const uint64_t datasetsize = dim0*dim1*sizeof(int);
	const uint64_t datasetsize = 0;

	if (datasetsize <= twogb) {
	...
	try {
		...
		DataSet* dataset = new DataSet(file->createDataSet(DATASETNAME, PredType::NATIVE_INT, dataspace, plist));
		GETTIME(endtime);
		float inittime = DIFF_TIME_SECS(starttime,endtime);
		printf("init time: %.3f\n", inittime);
